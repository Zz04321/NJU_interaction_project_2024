...
var minutes_per_step = [2581200, 5162400, 10324800, 25812000, 51624000, 258120000]; // minutes/格
var graduation_step = 20;//刻度间最小宽度，单位px
var hours_per_ruler = 2581200;//时间轴显示24小时
...
function mousewheelFunc(){
    ...
    if (delta < 0) {
        zoom = zoom + 2581200;
		if (zoom >= 10324800) {
			zoom = 10324800;//放大最大24小时
        }
        hours_per_ruler = zoom;
    } else if (delta > 0) {// 放大
        zoom = zoom - 2581200;
		if (zoom <= 2581200) {
			zoom = 2581200;//缩小最小1小时
        }
        hours_per_ruler = zoom;
    }
    clearCanvas();
